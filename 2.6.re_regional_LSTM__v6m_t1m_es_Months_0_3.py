...
if __name__ == "__main__":
    ...
    rmse_path = ['./rmse/rmse_re_regional_11_nan_avg_v6m_t1m_es_Month' + str(i+1) + '.csv' for i in range(12)]
    pred_path = ['./pred/pred_re_regional_11_nan_avg_0_v6m_t1m_es_Month' + str(i+1) + '.csv' for i in range(12)]
    ######################
    # re-index 34 points #
    ######################
    ...
    for month in range(0, 12):
        ...
        for idx in points_idx:
            vars_idx.extend([i for i in range((input_size+1) * idx, (input_size+1) * (idx + 1))])
        caqra_data_re = caqra_data.iloc[:, vars_idx]
        para_path = ['./para_re_regional/month'+str(month)+'_11_' + str(i) + '.pth' for i in range(point_nums)]
        # set rmse and pred
