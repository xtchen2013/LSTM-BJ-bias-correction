...
if __name__ == "__main__":
    ...
    validate_start = pd.to_datetime("2019-09-30 00:00:00", format="%Y-%m-%d %H:%M:%S")
    ...
    for exp in range(3):
        ...
        if exp == 0:
            input_size = 4
            para_path = [r'D:\beijing\para\para_simple_4_nan_avg_0_v3m_t1m\4_' + str(i) + '.pth' for i in
                         range(point_nums)]
            continue
        if exp == 1:
            input_size = 6
            para_path = [r'D:\beijing\para\para_simple_6_nan_avg_0_v3m_t1m\6_' + str(i) + '.pth' for i in
                         range(point_nums)]
            continue
        if exp == 2:
            input_size = 11
            para_path = [r'D:\beijing\para\para_simple_11_nan_avg_0_v3m_t1m\11_' + str(i) + '.pth' for i in
                         range(point_nums)]
        caqra_data = pd.read_csv(find_file('D:\\', caqra_path[exp]))
        # get 34 points
        points = ['东四', '天坛', '官园', '万寿西宫', '奥体中心', '农展馆', '万柳', '北部新区', '丰台花园', '云岗',
                  '古城', '房山', '大兴', '亦庄', '通州', '顺义', '昌平', '门头沟', '平谷', '怀柔', '密云', '延庆',
                  '定陵',
