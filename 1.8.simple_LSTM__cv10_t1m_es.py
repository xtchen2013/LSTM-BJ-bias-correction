...
# get mean and std #
####################
def get_mean_std(input_start, input_end):
    # get mean and std in training period for normalize and rescale
    df_train = load_data(input_start, input_end, 'train')[0]
    means = df_train.mean()
    stds = df_train.std()
    return means, stds
...
def simple_lstm():
    ...
    # cross validate data set up
    cv_date = pd.date_range(start=cv_start, end=cv_end, freq="H")
    tscv = TimeSeriesSplit(n_splits=10)
    for train_index, validate_index in tscv.split(cv_date):
        train_start = cv_date[train_index[0]]
        train_end = cv_date[train_index[-1]]
        validate_start = cv_date[validate_index[0]]
        validate_end = cv_date[validate_index[-1]]
        print(train_start, train_end, validate_start, validate_end)
        global mean, std
        mean, std = get_mean_std(train_start, train_end)
        train_data = DATA(train_start, train_end, 'train')  # training data
        train = DataLoader(train_data, batch_size=batch_size, shuffle=True)
        validate_data = DATA(validate_start, validate_end, 'validate')  # validation data
        validate = DataLoader(validate_data, batch_size=batch_size, shuffle=True)
        early_stopping = EarlyStopping(patience=20, verbose=True)  # early stop in 20 counters
        for i in range(epoch):
            ...
            # rmse.append(torch.sqrt(loss_func(obs, pred)))
            scheduler.step()
...
if __name__ == "__main__":
    ...
    # global dataset #
    ##################
    cv_start = pd.to_datetime("2014-01-01 00:00:00", format="%Y-%m-%d %H:%M:%S")
    cv_end = pd.to_datetime("2019-11-30 23:00:00", format="%Y-%m-%d %H:%M:%S")
    ...
    rmse_path = [r'D:\beijing\rmse\rmse_simple_4_nan_avg_0_v3m_t1m.csv',
                 r'D:\beijing\rmse\rmse_simple_6_nan_avg_0_v3m_t1m.csv',
                 r'D:\beijing\rmse\rmse_simple_11_nan_avg_v3m_t1m.csv']
    pred_path = [r'D:\beijing\pred\pred_simple_4_nan_avg_0_v3m_t1m.csv',
                 r'D:\beijing\pred\pred_simple_6_nan_avg_0_v3m_t1m.csv',
                 r'D:\beijing\pred\pred_simple_11_nan_avg_0_v3m_t1m.csv']
    obs_path = [r'D:\beijing\obs\obs_simple_4_nan_avg_0_v3m_t1m.csv',
                r'D:\beijing\obs\obs_simple_6_nan_avg_0_v3m_t1m.csv',
                r'D:\beijing\obs\obs_simple_11_nan_avg_0_v3m_t1m.csv']
    ###################
    # start go go go! #
    ###################
    for exp in range(3):
        ...
        if exp == 0:
            input_size = 4
            para_path = [r'D:\beijing\para\para_simple_4_nan_avg_0_v3m_t1m\4_' + str(i) + '.pth' for i in range(point_nums)]
            continue
        if exp == 1:
            input_size = 6
            para_path = [r'D:\beijing\para\para_simple_6_nan_avg_0_v3m_t1m\6_' + str(i) + '.pth' for i in range(point_nums)]
            continue
        if exp == 2:
            input_size = 11
            para_path = [r'D:\beijing\para\para_simple_11_nan_avg_0_v3m_t1m\11_' + str(i) + '.pth' for i in range(point_nums)]
        ...
        with tqdm(total=point_nums) as pbar:
            for num in range(34):
                point_num = num
                pred_simple, para_simple = simple_lstm()
