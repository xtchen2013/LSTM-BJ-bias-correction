...
def get_dataloader():
    ...
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=False, num_workers=10)
    # validation data
    validate_data = DATA(validate_start, validate_end, 'validate')
    validate_loader = DataLoader(validate_data, batch_size=batch_size, shuffle=False, num_workers=10)
    # test data
...
if __name__ == "__main__":
    ...
    train_end = pd.to_datetime("2018-06-30 23:00:00", format="%Y-%m-%d %H:%M:%S")
    validate_start = pd.to_datetime("2018-07-01 00:00:00", format="%Y-%m-%d %H:%M:%S")
    ...
    rmse_path = [r'D:\beijing\rmse\rmse_simple_4_nan_avg_0_v6m_t1m_es_Jan.csv',
                 r'D:\beijing\rmse\rmse_simple_6_nan_avg_0_v6m_t1m_es_Jan.csv',
                 r'D:\beijing\rmse\rmse_simple_11_nan_avg_v6m_t1m_es_Jan.csv']
    pred_path = [r'D:\beijing\pred\pred_simple_4_nan_avg_0_v6m_t1m_es_Jan.csv',
                 r'D:\beijing\pred\pred_simple_6_nan_avg_0_v6m_t1m_es_Jan.csv',
                 r'D:\beijing\pred\pred_simple_11_nan_avg_0_v6m_t1m_es_Jan.csv']
    obs_path = [r'D:\beijing\obs\obs_simple_4_nan_avg_0_v6m_t1m_es_Jan.csv',
                r'D:\beijing\obs\obs_simple_6_nan_avg_0_v6m_t1m_es_Jan.csv',
                r'D:\beijing\obs\obs_simple_11_nan_avg_0_v6m_t1m_es_Jan.csv']
    ###################
    # start go go go! #
    ###################
    for exp in range(3):
        ...
        if exp == 0:
            input_size = 4
            para_path = [r'D:\beijing\para\para_simple_4_nan_avg_0_v6m_t1m_es_Jan\4_' + str(i) + '.pth' for i in range(point_nums)]
            continue
        if exp == 1:
            input_size = 6
            para_path = [r'D:\beijing\para\para_simple_6_nan_avg_0_v6m_t1m_es_Jan\6_' + str(i) + '.pth' for i in range(point_nums)]
            continue
        if exp == 2:
            input_size = 11
            para_path = [r'D:\beijing\para\para_simple_11_nan_avg_0_v6m_t1m_es_Jan\11_' + str(i) + '.pth' for i in range(point_nums)]
