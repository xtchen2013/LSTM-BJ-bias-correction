...
def load_data(start, end, period):
    # load caqra data
    df = caqra_data
    df.index = pd.to_datetime([str(i) for i in df['date'].values], format="%Y-%m-%d %H:%M:%S")
    df = df.iloc[:, 2:]
    # variables + obs
...
#     def __init__(self, input_input_size, input_hidden_size, input_dropout_rate):
#         super(Model, self).__init__()
#         self.input_size = input_input_size      # = 6 (number of features)
#         self.seq_length = 720                   # time steps
#         self.flatten_size = self.input_size * self.seq_length  # 4320
#         self.hidden_size = input_hidden_size
#         self.dropout_rate = input_dropout_rate
#
#         self.fc1 = nn.Linear(self.flatten_size, self.hidden_size)
#         self.relu = nn.ReLU()
#         self.dropout = nn.Dropout(self.dropout_rate)
#         self.fc2 = nn.Linear(self.hidden_size, 1)  # regression output
#
#     def forward(self, x):
#         x = x.view(x.size(0), -1)  # flatten: (batch_size, 4320)
#         x = self.fc1(x)
#         x = self.relu(x)
#         x = self.dropout(x)
#         predict = self.fc2(x)
...
class Model(nn.Module):
    def __init__(self, input_input_size, input_hidden_size, input_dropout_rate):
        ...
        # CNN layers: conv over time axis
        self.conv1 = nn.Conv1d(in_channels=self.input_size, out_channels=32, kernel_size=5, padding=2)
        self.relu1 = nn.ReLU()
        self.pool1 = nn.MaxPool1d(kernel_size=4)  # downsample

        self.conv2 = nn.Conv1d(32, 64, kernel_size=3, padding=1)
        self.relu2 = nn.ReLU()
        self.pool2 = nn.AdaptiveAvgPool1d(output_size=1)  # output shape: (B, 64, 1)

        self.dropout = nn.Dropout(self.dropout_rate)
        self.fc = nn.Linear(64, 1)  # flatten to 1D regression output

    def forward(self, x):
        x = x.permute(0, 2, 1)  # from (B, 720, 6) → (B, 6, 720)
        x = self.relu1(self.conv1(x))  # → (B, 32, 720)
        x = self.pool1(x)              # → (B, 32, 180)
        x = self.relu2(self.conv2(x))  # → (B, 64, 180)
        x = self.pool2(x)              # → (B, 64, 1)
        x = x.squeeze(2)               # → (B, 64)
        x = self.dropout(x)
        predict = self.fc(x)           # → (B, 1)
        return predict
...
def get_dataloader():
    ...
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=False, num_workers=10, pin_memory=True)
    # validation data
    validate_data = DATA(validate_start, validate_end, 'validate')
    validate_loader = DataLoader(validate_data, batch_size=batch_size, shuffle=False, num_workers=10, pin_memory=True)
    # test data
...
if __name__ == "__main__":
    ...
    rmse_path = ['./rmse/rmse_cnn_4_nan_avg_0_v6m_t1m_es_Dec.csv',
                 './rmse/rmse_cnn_6_nan_avg_0_v6m_t1m_es_Dec.csv',
                 './rmse/rmse_cnn_11_nan_avg_v6m_t1m_es_Dec.csv']
    pred_path = ['./pred/pred_cnn_4_nan_avg_0_v6m_t1m_es_Dec.csv',
                 './pred/pred_cnn_6_nan_avg_0_v6m_t1m_es_Dec.csv',
                 './pred/pred_cnn_11_nan_avg_0_v6m_t1m_es_Dec.csv']
    # obs_path = [r'D:\beijing\obs\obs_simple_4_nan_avg_0_v6m_t1m_es_Dec.csv',
    #             r'D:\beijing\obs\obs_simple_6_nan_avg_0_v6m_t1m_es_Dec.csv',
    #             r'D:\beijing\obs\obs_simple_11_nan_avg_0_v6m_t1m_es_Dec.csv']
    ###################
    # start go go go! #
    ###################
    for exp in range(2, 3):
        ...
        if exp == 0:
            input_size = 4
            para_path = ['./para_cnn/para_cnn_4_nan_avg_0_v6m_t1m_es_Dec_4_' + str(i) + '.pth' for i in range(point_nums)]
        if exp == 1:
            input_size = 6
            para_path = ['./para_cnn/para_cnn_6_nan_avg_0_v6m_t1m_es_Dec_6_' + str(i) + '.pth' for i in range(point_nums)]
        if exp == 2:
            input_size = 11
            para_path = ['./para_cnn/para_cnn_11_nan_avg_0_v6m_t1m_es_Dec_11_' + str(i) + '.pth' for i in range(point_nums)]
        ...
        with tqdm(total=point_nums) as pbar:
            for num in range(34):
                ...
                rmse_simple, pred_simple, para_simple = simple_cnn()
